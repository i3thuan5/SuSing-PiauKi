    ...
    def 評詞陣列分(self, 詞陣列, 開始的所在=0):
        ...
        for 原本詞物件, ngram機率 in zip(詞性陣列[開始的所在:], self.詞性ngram.評詞陣列分(詞性陣列, 開始的所在)):
            ...
            try:
                ...
                規个語詞機率 = list(self.詞性語詞機率[原本詞物件.詞性].評詞陣列分([干焦型的詞物件]))[0]
...
#                 )
                yield ngram機率 + (規个語詞機率 * 0.5 + 拆詞機率 * 0.5)
...
def main():
    ...
    with open('結果.json') as 結果檔: 
        結果資料=json.load(結果檔)
    with open('答案.json') as 答案檔: 
        答案資料=json.load(答案檔)
    for 結果句,答案句 in zip(結果資料,答案資料):
        if 答案句['詞性']=='':
            毋知+1
            著
            毋著
